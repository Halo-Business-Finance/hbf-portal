...
const Index = () => {
  ...
  const handleSocialSignIn = async (provider: string, label: string) => {
    setSocialLoading(provider);
    setAuthError("");
    try {
      const redirectTo = getSafeRedirectUrl();
      const { error } = await authProvider.signInWithOAuth(provider as any, redirectTo ? { redirectTo } : undefined);
      if (error) {
        const msg = error.message || `Failed to sign in with ${label}`;
        setAuthError(msg);
        toast({ variant: "destructive", title: `${label} sign-in failed`, description: msg });
      }
    } catch (err) {
  ...
  const handleMicrosoftSignIn = () => handleSocialSignIn('azure', 'Microsoft');
  const handleGoogleSignIn = () => handleSocialSignIn('google', 'Google');
  const handleAppleSignIn = () => handleSocialSignIn('apple', 'Apple');
  const handleLinkedInSignIn = () => handleSocialSignIn('linkedin_oidc', 'LinkedIn');
  const resetForm = () => {
    setEmail("");
    setPassword("");
    setConfirmPassword("");
        ...
        backgroundImage: 'none'
          ...
          <div className="relative z-10 w-full max-w-lg bg-white md:rounded-2xl md:shadow-2xl p-6 sm:p-8 md:p-10">
            ...
            <form onSubmit={handleAuthSubmit} className="space-y-5" aria-label={isLogin ? "Sign in form" : "Create account form"}>
              ...
              {/* Sign On Button */}
              <Button type="submit" className="w-full h-12 !bg-[#1a2456] text-white rounded-full font-bold hover:!bg-[#152050] transition-colors flex items-center justify-center gap-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:!opacity-90 disabled:cursor-not-allowed" disabled={authLoading || isLockedOut || !email || !password}>
                {authLoading ? <span className="flex items-center gap-2 justify-center text-white font-bold">
                    ...
                    <span className="text-white font-bold">{isLogin ? "Sign In" : "Create account"}</span>
                  </>}
              </Button>
            ...
            {isLogin && <div className="mt-2 mb-[-16px] text-center px-4">
