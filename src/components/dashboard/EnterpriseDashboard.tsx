...
export const EnterpriseDashboard = ({ onNewApplication }: EnterpriseDashboardProps) => {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig: Record<string, { label: string; className: string }> = {
      ...
      under_review: { label: 'Under Review', className: 'bg-amber-500 text-white' },
      approved: { label: 'Approved', className: 'bg-green-500 text-white' },
      funded: { label: 'Funded', className: 'bg-emerald-600 text-white' },
      rejected: { label: 'Declined', className: 'bg-red-500 text-white' }
    };
  ...
  const quickActions = [
    { label: 'Tax documents', icon: FileText, action: () => navigate('/my-documents') },
    { label: 'New Application', icon: Plus, action: () => onNewApplication?.(), chevron: true },
    { label: 'Upload Documents', icon: Upload, action: () => navigate('/my-documents'), chevron: true },
    { label: 'Loan Calculator', icon: Calculator, action: () => navigate('/loan-calculator'), chevron: true },
    { label: 'Credit Reports', icon: CreditCard, action: () => navigate('/credit-reports'), chevron: true },
    { label: 'More', icon: MoreHorizontal, action: () => navigate('/loan-applications') }
  ];
      ...
      <div className="flex flex-wrap items-center justify-end gap-2 md:gap-3">
        {quickActions.map((action, index) => (
          <Button
            ...
            variant="default"
            size="sm"
            className="rounded-full px-4 py-2 h-9 font-medium bg-primary hover:bg-primary/90 text-primary-foreground"
        ...
        <div className="space-y-6">
          ...
          <Card className="border border-border">
            <CardHeader className="pb-2">
              <CardTitle className="text-lg font-bold">Pipeline last 30 days</CardTitle>
            </CardHeader>
          ...
          <Card className="border border-border">
            ...
            <CardContent className="space-y-1">
              <Button 
                variant="ghost" 
                className="w-full justify-between h-auto py-3 px-2 hover:bg-muted"
